...
- 调整三拍对撞覆盖层节奏：主角水平冲刺到目标魔物旁后，进入动作序列前的停顿从 1 秒缩短为 0.5 秒，保持动作间隔仍为 0.5 秒。更新 [[design/03_experience/00_ui_ux|界面与交互]]。
- 优化三拍对撞覆盖层表现：创建主角与目标魔物副本后隐藏背景中的真实主角、真实魔物和真实魔物拍位；覆盖层目标魔物补充血条和拍位 UI，血条随每段伤害实时扣减；主角冲刺改为只做水平移动，固定停在目标魔物左侧 100px 外；同目标连续拍省略拍间冲刺，跨目标回位时主角面朝左。更新 [[design/03_experience/00_ui_ux|界面与交互]]。
- 修改多段动作拍位的结算与演出契约：规则层为每个动作阶段输出结构化阶段结果，表现层按阶段播放动画、弹出伤害数字和格挡反馈；若多段攻击中途击杀目标，后续阶段不再结算、不产彩、不播放，当前拍位直接结束。更新 [[design/01_core_gameplay/02_combat_system|战斗系统]] 与 [[design/03_experience/00_ui_ux|界面与交互]]。
- 为三拍对撞特写覆盖层接入主角动作动画：移动过程播放带剑跑步序列帧，行动牌动作预留斩击、钝击、弹射、闪避、格挡五类动画接口，当前先登记并轮换播放斩击三组连招资源。更新 [[design/03_experience/00_ui_ux|界面与交互]]。
- 调整主角对撞动作资源处理方式：移动和斩击动画改为导入前预裁剪 spritesheet，运行时只按已登记行列播放，不再通过表现层代码按 alpha 边界动态裁剪。
- 优化拍位对撞视觉流程：结算期间隐藏手牌 UI；每个目标段先隐藏非目标魔物及其魔物拍位，再按真实界面位置和尺寸生成主角、目标魔物副本；主角冲刺到目标身边后停顿 1 秒，逐个播放该拍位动作，动作间隔 0.5 秒；连续拍位锁定同一魔物时复用当前覆盖层，跨目标时才关闭覆盖层并恢复隐藏节点。
...
- 补充行动牌附魔预览、终结牌颜色消耗预览、多色终结牌结算拆解和黄色 / 紫色高复杂触发日志提示。同步更新 [[design/03_experience/00_ui_ux|界面与交互]]。
- 补充色彩增强平衡边界，明确黄色、紫色、绿色、蓝色和防御转伤害的高风险控制原则。同步更新 [[design/04_balance_data/00_balance_principles|平衡原则]] 与 [[design/08_governance/02_glossary|术语表]]。
