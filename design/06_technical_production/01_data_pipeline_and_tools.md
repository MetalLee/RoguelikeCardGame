...
- 单张卡牌插画使用稳定资源键或 `art_key` 关联内容 ID；卡牌名称、规则文本、数值和本地化文本仍从 `game/data/` 与本地化键读取。第一版 MVP 中已归档全部行动牌、技能牌和终结牌卡面，资源键采用 `art.card.<card_id_without_prefix>` 形式，例如 `art.card.basic_strike`。
