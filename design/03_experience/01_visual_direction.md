...
- 默认战斗界面气氛应保持明亮、温暖、可读，避免把 UI 和背景推向暗黑哥特、恐怖或重黑装饰方向。
...
该图用于统一第一版 MVP 战斗界面的整体构图、明亮漫画书气氛、HUD 信息层级、角色 / 敌人舞台比例、手牌扇形布局和连锁进度表达。它与 [[game/assets/art/style_guides/mvp_key_elements_visual_master.png|第一版 MVP 关键元素视觉母版]] 的关系是：视觉母版定义角色、敌人、卡牌、牌包和遗物的单体风格；战斗场景设计标准定义这些元素在实际战斗界面中的组合方式。
...
- 手牌保持竖版卡牌比例并在底部以紧凑扇形叠放；行动牌、技能牌和终结牌继续沿用血红、冷青 / 幽蓝、紫黑主题。
- 连锁 UI 使用 `8` 格点状进度作为主视觉，`3 / 5 / 8` 阈值通过弱提示、卡牌状态或达成 VFX 表达，不作为中上方 HUD 的大号数字焦点。
