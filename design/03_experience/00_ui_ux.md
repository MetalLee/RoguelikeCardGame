...
MVP 战斗界面默认按 PC 16:9 横屏设计：玩家生命、防御和状态保持在左上稳定区；敌人名称、生命、防御、状态和意图保持在右上稳定区；行动点与抽牌堆保持在左下；弃牌堆与结束回合按钮保持在右下；手牌位于底部中间，并以紧凑扇形叠放；当前连锁层数位于中上方，使用 8 格连锁点进度作为主要表达。
...
- 左上角只显示玩家生命、防御和状态条，不显示主角头像；右上角只显示敌人名称、生命、防御、状态和意图，不显示敌人头像。
- 左下角显示当前行动点与抽牌堆图标 / 数量；右下角显示弃牌堆图标 / 数量与结束回合按钮。
- 中间下方为手牌区，手牌使用竖版卡牌比例，紧凑叠放并以扇形展开，中心牌可以轻微抬高用于悬停或选中反馈。
- 第一版 MVP 的手牌出牌交互采用拖拽释放：悬停卡牌时仅抬升并置顶，不触发布牌；行动牌、技能牌、终结牌都需要按住拖拽后释放才会请求出牌。`TargetRule.Self`、`TargetRule.AllEnemies` 和 `TargetRule.None` 卡牌释放到主角、敌人和手牌之间的中场区域才可出牌；`TargetRule.SingleEnemy` 卡牌必须用漫画风箭头明确指向有效敌人目标后释放，不能只拖到中场区域出牌。表现层只负责 hover、drag、箭头、释放区和目标高亮，费用、连锁、目标合法性和胜负仍由规则层校验。
