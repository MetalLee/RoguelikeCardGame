...
敌人立绘应作为透明舞台层融入战斗背景，不应被放入大面积黑色卡片或面板中。敌人状态信息可以使用轻量纸质状态条承载；目标选中、意图和受击反馈通过图标、边缘高亮和 VFX 表达。
...
奖励界面第一版动效覆盖卡牌包打开、候选卡依次弹出和选卡确认反馈。该阶段不要求完整逐帧角色动作、骨骼动画或镜头时间轴；后续若增加主角 / 敌人动作，应继续由同一套事件驱动机制触发。
